...
export class StudentService {
  ...
  // get a /api/student/{id} -> devuelve un Observable con un solo estudiante por su ID
  getStudentById(id: number): Observable<Student> {
    return this.http.get<Student>(`${this.apiUrl}/${id}`);
  }

  // POST a /api/student -> crea un nuevo estudiante y devuelve el objeto con su ID asignado
  createStudent(student: Omit<Student, 'id'>): Observable<Student> {
    return this.http.post<Student>(this.apiUrl, student);
  }

  // DELETE a /api/student/{id} -> elimina un estudiante por su ID
