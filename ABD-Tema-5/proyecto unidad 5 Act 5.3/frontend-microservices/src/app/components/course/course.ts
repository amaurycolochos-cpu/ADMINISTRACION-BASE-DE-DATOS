...
export class Course implements OnInit {
  ...
  loadCourses(): void {
    ...
    this.courseService.getCourses().subscribe({
      ...
      error: (err) => {
        ...
        console.error('Error al consumir el microservicio de cursos: - course.ts:55', err);
        this.error.set(
  ...
  createCourse(): void {
    ...
    this.courseService.createCourse({ name, description }).subscribe({
      ...
      error: (err) => {
        console.error('Error al crear curso: - course.ts:89', err);
  ...
  searchById(): void {
    ...
    this.courseService.getCourseById(id).subscribe({
      ...
      error: (err) => {
        console.error('Error al buscar curso por ID: - course.ts:114', err);
        this.foundCourse.set(null);
